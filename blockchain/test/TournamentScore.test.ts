...
describe("TournamentScore (for Match Results)", function () {
  ...
  beforeEach(async function () {
    ...
    tournamentScore = await TournamentScoreFactory.deploy(owner.address) as TournamentScore;
  ...
  describe("recordMatchResult", function () {
    it("Should record a match result correctly by the owner", async function () {
      ...
      await tournamentScore.connect(owner).recordMatchResult(
        matchId,
        tournamentId,
        player1Id,
        player2Id,
    ...
    it("Should fail if trying to record the same match twice", async function () {
      const matchId = "match-uuid-2";
      await tournamentScore.recordMatchResult(matchId, "t1", "p1", "p2", 1, 0, "p1");

      await expect(
        tournamentScore.recordMatchResult(matchId, "t1", "p1", "p2", 2, 0, "p1")
    ...
    it("Should fail if called by an account other than the owner", async function () {
      ...
      await expect(
        tournamentScore.connect(otherAccount).recordMatchResult(
          matchId, "t1", "p1", "p2", 1, 0, "p1"
        )
