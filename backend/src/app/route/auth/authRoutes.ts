...
const userRepo = new TypeOrmUserRepository(AppDataSource.getRepository(UserEntity));
...
const authRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  // 2FA セットアップ
  fastify.post<{ Body: { email?: string } }>('/2fa/setup', async (request, reply) => {
  ...
  fastify.post<{ Body: TwoFABody }>('/2fa/verify', async (request, reply) => {
    ...
    if (!email || !token) return reply.code(400).send({ message: 'Email and token required' });
