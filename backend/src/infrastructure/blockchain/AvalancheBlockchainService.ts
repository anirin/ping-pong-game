...
import TournamentScoreABI from "../abi/contracts/TournamentScore.sol/TournamentScore.json" with { type: "json" };
...
export class AvalancheBlockchainService {
	...
	private constructor() {
		...
		if (!rpcUrl || !privateKey || !contractAddress) { // ★★★ rpcUrlもチェック対象に ★★★
			console.error("[Blockchain] Service not initialized: Missing environment variables.");
			return;
		}
		...
		try {
            ...
            console.log("AvalancheBlockchainService initialized successfully (using in-memory Hardhat Network).");
	...
	public async recordMatchResult(
		...
		try {
			if (typeof this.contract.recordMatchResult !== 'function') {
				throw new Error("The 'recordMatchResult' function does not exist on the smart contract.");
			}

			console.log(`[Blockchain] Recording match result for ID: ${matchId}`);
			
			const txResponse: ethers.TransactionResponse = await this.contract.recordMatchResult(
				matchId,
				tournamentId,
				player1Id,
				player2Id,
			...
			console.error("[Blockchain] Error recording match result:", error);
