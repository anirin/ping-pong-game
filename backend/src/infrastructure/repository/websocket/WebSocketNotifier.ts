...
import type { MatchFinishDto, RealtimeMatchStateDto } from "@application/services/game/MatchData.js";
    ...
    public removeConnection(matchId: MatchId, socket: WebSocket): void {
        ...
        if (room) {
            ...
            if (room.size === 0) {
                ...
                console.log(`[WebSocket] Match room ${matchId} is now empty and has been deleted.`);
            }
        }
    }

    public broadcastGameState(matchId: MatchId, state: RealtimeMatchStateDto): void {
        const room = this.matchRooms.get(matchId);
        if (!room) return;
