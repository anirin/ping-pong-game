...
export class TournamentController {
	...
	private updatePathColors(dummyData: {
		...
		matches.forEach((match) => {
			...
			console.log(
				`Comparing ${match.user1} (${score1}) vs ${match.user2} (${score2})`,
			);
			...
			if (score1 > score2) {
				if (path1) {
					...
					console.log(
						`Winner: ${match.user1} (${score1}) - ${match.path1} colored red`,
					);
				}
				...
				if (path2) {
					...
					console.log(
						`Winner: ${match.user2} (${score2}) - ${match.path2} colored red`,
					);
				}
			} else {
