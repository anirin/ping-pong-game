...
export class GuestMatchController {
	...
	private async loadMatchInfoFromGlobalState(): Promise<void> {
		try {
			const { TournamentStateManager } = await import("../../tournament_guest/ui/tournamentState.js");
			...
			if (currentMatch) {
				...
				console.warn("グローバル状態にマッチ情報が見つかりません。デフォルト値を使用します。");
			...
			console.error("グローバル状態からマッチ情報を取得できませんでした:", error);
			this.matchId = "unknown-match";
			this.player1 = "Player 1";
			this.player2 = "Player 2";
			this.round = 1;
	...
	private updateGame(): void {
		// プレイヤー1（左側）のパドル移動
		if (this.player1MovingUp && this.gameState.paddle1Y > CONSTANTS.PADDLE_MIN_Y) {
			this.gameState.paddle1Y -= CONSTANTS.PADDLE_SPEED;
		}
		if (this.player1MovingDown && this.gameState.paddle1Y < CONSTANTS.PADDLE_MAX_Y) {
			this.gameState.paddle1Y += CONSTANTS.PADDLE_SPEED;
		}

		// プレイヤー2（右側）のパドル移動
		if (this.player2MovingUp && this.gameState.paddle2Y > CONSTANTS.PADDLE_MIN_Y) {
			this.gameState.paddle2Y -= CONSTANTS.PADDLE_SPEED;
		}
		if (this.player2MovingDown && this.gameState.paddle2Y < CONSTANTS.PADDLE_MAX_Y) {
			this.gameState.paddle2Y += CONSTANTS.PADDLE_SPEED;
		}
	...
	private checkPaddleCollision(): void {
		...
		// 左パドルとの衝突
		if (
			this.gameState.ballX - CONSTANTS.BALL_RADIUS <= CONSTANTS.PADDLE_MARGIN + CONSTANTS.PADDLE_WIDTH &&
			this.gameState.ballX - CONSTANTS.BALL_RADIUS >= CONSTANTS.PADDLE_MARGIN &&
			this.gameState.ballY >= this.gameState.paddle1Y &&
			this.gameState.ballY <= this.gameState.paddle1Y + CONSTANTS.PADDLE_HEIGHT &&
			this.gameState.ballSpeedX < 0
		) {
			this.gameState.ballSpeedX = -this.gameState.ballSpeedX;
			// パドルの位置に応じてボールの角度を調整
			const hitPos = (this.gameState.ballY - this.gameState.paddle1Y) / CONSTANTS.PADDLE_HEIGHT;
			this.gameState.ballSpeedY = (hitPos - 0.5) * 10;
		}

		// 右パドルとの衝突
		if (
			this.gameState.ballX + CONSTANTS.BALL_RADIUS >= canvas.width - CONSTANTS.PADDLE_MARGIN - CONSTANTS.PADDLE_WIDTH &&
			this.gameState.ballX + CONSTANTS.BALL_RADIUS <= canvas.width - CONSTANTS.PADDLE_MARGIN &&
			this.gameState.ballY >= this.gameState.paddle2Y &&
			this.gameState.ballY <= this.gameState.paddle2Y + CONSTANTS.PADDLE_HEIGHT &&
			this.gameState.ballSpeedX > 0
		) {
			this.gameState.ballSpeedX = -this.gameState.ballSpeedX;
			// パドルの位置に応じてボールの角度を調整
			const hitPos = (this.gameState.ballY - this.gameState.paddle2Y) / CONSTANTS.PADDLE_HEIGHT;
			this.gameState.ballSpeedY = (hitPos - 0.5) * 10;
		}
	}
	...
	private checkWallCollision(): void {
		...
		// 上下の壁との衝突
		if (this.gameState.ballY - CONSTANTS.BALL_RADIUS <= 0 || this.gameState.ballY + CONSTANTS.BALL_RADIUS >= canvas.height) {
	...
	private endGame(): void {
		...
		const winner = this.gameState.score1 > this.gameState.score2 ? this.player1 : this.player2;
	...
	private drawGame(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement): void {
		// 背景をクリア
		ctx.fillStyle = "#000";
		ctx.fillRect(0, 0, canvas.width, canvas.height);
		...
		// ボールを描画
		ctx.beginPath();
		ctx.arc(this.gameState.ballX, this.gameState.ballY, CONSTANTS.BALL_RADIUS, 0, Math.PI * 2);
		ctx.fill();
